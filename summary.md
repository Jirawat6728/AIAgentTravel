...
> **Version**: 1.1.0 | **Architecture**: Two-Pass ReAct + LangGraph | **Last Updated**: 3 March 2026
...
| Cache | **In-process memory** (Python dict, TTL 24h) |
...
| Styling | Pure CSS (ไม่มี Tailwind/MUI) + Theme (Light/Dark ผ่าน ThemeContext + theme-content.css) |
...
│   ├── docs/
│   │   ├── VISA_DESIGN.md         # เอกสารออกแบบระบบวีซ่า
│   │   ├── PASSPORT_DESIGN.md     # ออกแบบระบบพาสปอร์ตหลายเล่ม
│   │   └── FARE_RULES_DESIGN.md   # โครงข้อมูล Fare Rules / Fare Details (Refund, Change, Check-in, Baggage)
│   └── app/
...
เงื่อนไขตั๋ว (Refund, Change, Check-in, Baggage, Fare Type) ออกแบบเป็น canonical schema ใน **`backend/docs/FARE_RULES_DESIGN.md`** — ใช้กับ `raw_data.enhanced_info` (data_aggregator) และ `flight_details` (chat.py → frontend). รองรับการแสดง Badge คืนเงินได้/ไม่ได้, สรุปเงื่อนไขก่อนชำระเงิน และการ map จาก Amadeus (pricingOptions, fareDetailsBySegment, Fare Rules API ในอนาคต).
